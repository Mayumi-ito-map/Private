"""
司令塔②（ベースライン版）: 同義語展開なし・行単位指標あり

同義語展開を行う前の行単位指標（rows_with_hit, rows_still_0）を把握するため、
expand_synonyms を呼ばず、Stage1→2→3 のみでマッチングする版。
出力先: output_match_results_baseline / world_summary.xlsx

比較用:
  1. 本スクリプトを実行 → ベースラインの行単位指標を確認
  2. run_stage_match.py を実行（同義語展開あり）→ 差分で同義語展開の効果を確認
"""
...
OUTPUT_DIR = BASE_DIR / "output_match_results_baseline"
...
def main():
    ...
    if stats:
        ...
        print("world_summary.xlsx saved -> output_match_results_baseline/")
